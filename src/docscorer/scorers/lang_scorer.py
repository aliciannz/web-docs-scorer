    ...
    def score(
        ...
        if correct_lang_chars == 0:
            if not available_chars:
                print(
                    f"Doc_name: '{id}' -No available segments have been found on the target language -Language: '{ref_language}' -Segment_languages: {set(lang_segments)}"
                )
            else:
                print(
                    f"Doc_name: '{id}' -Only too short segments have been found on the target language"
                )
            return 0
